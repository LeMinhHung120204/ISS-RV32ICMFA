...
module DataMem_wrapper #(
    parameter FF_DEPTH      = 32            // Number of RAM entries (must be power of 2)
,   parameter RAM_ADDR_W    = 16   // Address width for RAM
// ,   parameter ID_W          = 2
,   parameter DATA_W        = 32
,   parameter ADDR_W        = 32
...
,   input               i_axi_rready
    ...
    wire                    fifo_aw_pop, fifo_w_pop;
    wire                    fifo_b_pop, fifo_b_push;
    wire                    fifo_ar_pop, fifo_r_pop;
    ...
    assign o_axi_bresp   = 2'b00; // Always OKAY since no errors in this simple design
    ...
    ,   .din    ({i_axi_awaddr, i_axi_awlen, i_axi_awsize, i_axi_awburst})
    ,   .empty  (fifo_aw_empty)
    ,   .full   (fifo_aw_full)
    ,   .dout   (fifo_aw_dout)
    );
    ...
    ,   .din    ({i_axi_araddr, i_axi_arlen, i_axi_arsize, i_axi_arburst})
    ,   .empty  (fifo_ar_empty)
    ,   .full   (fifo_ar_full)
    ,   .dout   (fifo_ar_dout)
    );
    ...
    ,   .din    ({ram_r_data, 2'b00, last_data_from_mem})
    ,   .empty  (fifo_r_empty)
    ,   .full   (fifo_r_full)
    ,   .dout   (fifo_r_dout)
    );
