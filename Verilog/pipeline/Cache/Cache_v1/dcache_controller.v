        ...
        case (state)
            TAG_CHECK: begin
                if (!cpu_req || (i_atomic_sc && (!res_valid || !res_hit))) begin
                    ...
                    if (i_atomic_sc && cpu_req) sc_done = 1'b1;
                end 
                    ...
                    if (is_valid && victim_dirty) next_state = WB_SEND;
            ...
            WB_SEND: begin
                ...
                if (i_req_ready) next_state = WB_WAIT;
            end

            WB_WAIT: begin
                if (i_resp_valid) next_state = BUS_REQ; // Writeback xong, quay lại lấy Block mới
            end

            BUS_REQ: begin
                o_req_valid = 1'b1;
                if (is_write && hit) o_req_cmd = CMD_UPGRADE;
                else if (is_write && !hit) o_req_cmd = CMD_READ_UNIQUE;
                else o_req_cmd = CMD_READ_SHARED;
                
                if (i_req_ready) next_state = BUS_WAIT;
            end

            BUS_WAIT: begin
                if (i_resp_valid) next_state = UPDATE;
            end

            UPDATE: begin // Nhận cờ hiệu về, cập nhật SRAM
                tag_we    = 1'b1;
                moesi_we  = 1'b1;
                if (!(is_write && hit)) refill_we = 1'b1; // Refill Data khi Miss
                
                // Nếu là AMO mà miss -> Vừa lấy đc data về -> qua AMO Ghi đè vào Data
                if (i_atomic_amo) next_state = AMO_EXEC;
                else next_state = TAG_CHECK;
            end
