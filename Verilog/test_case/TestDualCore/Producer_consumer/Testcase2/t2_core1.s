...
wait_empty:
    # Lệnh amoand.w: M = M & 0. Trả về giá trị cũ vào x14.
    # Nếu x14 = 1 (trống), ta đã thành công xóa nó về 0.
    amoand.w  x14, x0, (x17) 
    beq       x14, x0, wait_empty   

    # --- CRITICAL SECTION: SẢN XUẤT ---
    addi      x13, x11, 100
    sw        x13, 0(x10)        # Ghi Data

    # Lệnh amoor.w: M = M | 1. Bật cờ Full lên 1.
    amoor.w   x0, x16, (x18) 

    # Tăng biến đếm và lặp lại
    addi      x11, x11, 1        
    j         producer_loop
